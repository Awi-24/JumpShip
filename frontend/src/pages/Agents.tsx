...
import type { Agent, AgentLogEntry, AgentStatus, AgentInteraction } from '../types';
...
function statusLabel(status: AgentStatus): string {
  switch (status) {
    ...
    case 'review_requested': return '👁 Review Required';
    ...
    <div className="agent-detail-overlay">
      <div className="interaction-dialog" onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          <span style={{ fontSize: 22 }}>{isReview ? '👁' : '🆘'}</span>
...
function ModelCheckBadge({ label, value }: { label: string; value: boolean | null }) {
  ...
  return (
    <span
      className="model-check-badge"
      style={{ background: value ? 'rgba(74,222,128,0.15)' : 'rgba(248,113,113,0.15)', color: value ? '#4ade80' : '#f87171' }}
    >
      {value ? '✓' : '✗'} {label}
    </span>
  );
    ...
    <div className="agent-card" onClick={onSelect} style={{ outline: needsInteraction ? '2px solid var(--gold)' : undefined }}>
      ...
      <div className="agent-card-body">
        <div className="agent-card-top">
          ...
          <span className="agent-status-badge" style={{ color: statusColor(agent.status) }}>
        ...
        {needsInteraction && agent.interaction_pending && (
          ...
          >
            <div style={{ marginBottom: 8, fontWeight: 600 }}>
              {agent.status === 'review_requested' ? '👁 Review Required' : '🆘 Help Requested'}
            </div>
        ...
        <div className="agent-card-actions" onClick={e => e.stopPropagation()}>
          ...
          <button className="btn-agent-view" onClick={onSelect}>View →</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="agent-detail-panel" onClick={e => e.stopPropagation()}>
        ...
        <div className="agent-detail-header">
          ...
          <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
            <span className="agent-status-badge" style={{ color: statusColor(liveAgent.status), fontSize: 15 }}>
        ...
        {needsInteraction && liveAgent.interaction_pending && (
          ...
          >
            <div style={{ fontWeight: 700, color: 'var(--gold)', marginBottom: 8 }}>
              {liveAgent.status === 'review_requested' ? '👁 Review Required' : '🆘 Help Requested'}
            </div>
...
const LLM_PROVIDERS = ['', 'ollama', 'anthropic', 'openai', 'groq', 'lmstudio'] as const;
...
function StartAgentModal({
  ...
  onClose: () => void;
  onStart: (url: string, title: string, company: string, llmConfig: Record<string, string>) => void;
}) {
  ...
  const isLocal = llmProvider === 'ollama' || llmProvider === 'lmstudio';
  ...
  const checkCaps = async () => {
    ...
    setCaps(null);
    try {
      const params = new URLSearchParams({ provider: llmProvider, model: llmModel });
      if (llmApiKey) params.set('api_key', llmApiKey);
      if (llmBaseUrl) params.set('base_url', llmBaseUrl);
      ...
      <div className="agent-start-modal" onClick={e => e.stopPropagation()}>
        ...
        <div style={{ borderTop: '1px solid var(--border)', margin: '16px 0', paddingTop: 16 }}>
          <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-muted)', letterSpacing: '0.08em', marginBottom: 12 }}>
            LLM CONFIGURATION (optional — overrides global settings)
          </div>
          <div className="profile-field">
...
export default function Agents({ onBack }: AgentsProps) {
  ...
  const handleStart = async (url: string, title: string, company: string, llmConfig: Record<string, string>) => {
    ...
    <div className="agents-page">
      ...
      <header className="agents-header">
        ...
        <div className="agents-header-right">
          <div className="agents-stats">
            <span className="agent-stat running">⚡ {running} running</span>
            <span className="agent-stat completed">✓ {completed} done</span>
            <span className="agent-stat failed">✗ {failed} failed</span>
            {waiting > 0 && <span className="agent-stat" style={{ color: 'var(--gold)' }}>👁 {waiting} waiting</span>}
          </div>
      ...
      {agents.length === 0 ? (
        <div className="agents-empty">
          ...
          <button className="btn-primary" onClick={() => setShowStartModal(true)} style={{ marginTop: 20 }}>
